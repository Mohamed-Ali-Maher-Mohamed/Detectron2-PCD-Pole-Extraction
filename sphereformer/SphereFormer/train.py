...
from util.siemens import SiemensDataset
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    elif args.data_name == 'siemens':
        train_data = SiemensDataset(args.data_root,
                                voxel_size=args.voxel_size,
                                split='test',
                                voxel_max=args.voxel_max,
                                xyz_norm=args.xyz_norm)
    ...
    if args.data_name == 'nuscenes':
        ...
        val_data = SiemensDataset(args.data_root,
                                voxel_size=args.voxel_size,
                                split='test',
                                voxel_max=args.voxel_max,
                                xyz_norm=args.xyz_norm)
...
def validate(val_loader, model, criterion):
    ...
    for i, batch_data in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            out_dir = 'output/test_kitti'
            if not os.path.exists(out_dir):
                os.mkdir(out_dir)

            np.save(os.path.join(out_dir, Path(filename[0]).stem + '.npy'), output.detach().cpu().numpy())
